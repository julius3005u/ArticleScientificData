"""Signal Viewer - Interactive visualization of generated signals with metadata."""
...
class FilePicker:
    ...
    def show_dialog(self):
        ...
        # Get default path (parent directory)
...
class SignalViewer:
    """Main application for viewing signals and their metadata."""
    ...
    def setup_ui(self):
        ...
        self.metadata_frame = ttk.LabelFrame(content_frame, text="Metadata", padding=10)
        self.metadata_frame.pack(side=tk.RIGHT, fill=tk.BOTH, padx=(10, 0))
        self.metadata_frame.configure(width=350)
    ...
    def load_signal(self, index: int):
        ...
        Args:
            index: Index in the signals array
        """
    ...
    def display_metadata(self, index: int, signal: np.ndarray):
        """Display metadata in the right panel.
        ...
        for label, value in properties:
            ...
            value_label = tk.Label(frame, text=str(value), font=("Arial", 11), foreground="#00FF00", background="#2a2a2a")
            value_label.pack(side=tk.LEFT, fill=tk.X, expand=True)
    
    def plot_signal_old(self, signal: np.ndarray, index: int):
        """Plot the signal using matplotlib.
        ...
        src_label.pack(anchor=tk.W, pady=(0, 10), fill=tk.X, padx=5)
        ...
        key_fields = [
            ("Time Range", f"{format_value(metadata.get('t_start', 'N/A'))} - {format_value(metadata.get('t_end', 'N/A'))} s"),
            ("Sampling Rate", f"{format_value(metadata.get('fs_high', 'N/A'))} Hz"),
            ("Tau Frequency", format_value(metadata.get('tau_frequency', 'N/A'))),
            ("Tau Amplitude", format_value(metadata.get('tau_amplitude', 'N/A'))),
            ("Amplitude Spline", format_value(metadata.get('amplitude_spline_type', 'N/A'))),
            ("Vertical Offset", format_value(metadata.get('vertical_offset', 'N/A'))),
        ]
        ...
        # Noise profile
        sep2 = ttk.Separator(self.metadata_content, orient=tk.HORIZONTAL)
        sep2.pack(fill=tk.X, pady=5)
        
        noise_title = tk.Label(
            self.metadata_content,
            text="Noise Profile",
            font=("Arial", 12, "bold"),
            foreground="white",
        ...
        noise_title.pack(anchor=tk.W, pady=(10, 5), fill=tk.X, padx=5)
        
        noise_profile = metadata.get("noise_profile", {})
        noise_fields = [
            ("Has Noise", format_value(noise_profile.get('has_noise', 'N/A'))),
            ("Noise Type", format_value(noise_profile.get('noise_type', 'N/A'))),
            ("Noise Std", format_value(noise_profile.get('noise_std', 'N/A'))),
        ]
        
        for label, value in noise_fields:
            frame = tk.Frame(self.metadata_content, background="#2a2a2a")
        ...
        sep3 = ttk.Separator(self.metadata_content, orient=tk.HORIZONTAL)
        sep3.pack(fill=tk.X, pady=5)
        
        def show_all_metadata():
            """Show all metadata in a formatted view."""
            all_meta = json.dumps(metadata, indent=2, default=str)
            popup = tk.Toplevel(self.root)
            popup.title(f"All Metadata - {signal_id}")
            popup.geometry("600x400")
            
            text = tk.Text(popup, wrap=tk.WORD)
            text.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
            text.insert(tk.END, all_meta)
            text.config(state=tk.DISABLED)
        
        btn = tk.Button(
            self.metadata_content,
            text="Show All Metadata",
            command=show_all_metadata,
            font=("Arial", 11),
            foreground="white",
            background="#888888",
            activeforeground="white",
