"""Noise profile utilities for SignalBuilderCLI.
...
def apply_noise_profile(
    ...
    Gaussian case uses localized strategies (central time window 
    vs. high-variation segments).
    ...
    clean_signal : np.ndarray
        The clean signal to add noise to.
    t_high : np.ndarray
    ...
    t_end : float
        Signal end time.
    base_points : list
        Frequency profile base points.
    variation_type : list
        List of variation types per segment.
    config : NoiseProfileConfig
    ...
    if has_noise_draw >= config.p_has_noise:
        ...
        return clean_signal.copy(), noise_metadata
...
def _apply_structured_noise(
    ...
    segments of the signal. Parameters:
